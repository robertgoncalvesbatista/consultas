...
export async function cadastroPessoaFisicaAction(data: { cpf: string }) {
  ...
  try {
    ...
    const cadastroPessoaFisica = await db.cadastroPessoaFisicaBasico.findFirst({
      where: { cpf: cpfSearch },
    });
    ...
    if (!cadastroPessoaFisica) {
      ...
      return await db.cadastroPessoaFisicaBasico.create({
    ...
    if (cadastroPessoaFisica.updatedAt <= aMonthAgo) {
      ...
      return await db.cadastroPessoaFisicaBasico.update({
